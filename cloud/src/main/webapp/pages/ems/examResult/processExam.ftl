...
<script type="text/template" id="detailDetailTableTemp">
    <table class="pe-stand-table pe-stand-table-default">
        ...
        <%_.each(peData.rows,function(user,index){%>
        <tr>
            <%if (peData.tableTitles[0].name === 'ticket') {%>
            <td title="<%=user.ticket%>"><%=user.ticket%></td>
            <%}%>
            <td title="<%=user.userName%>"><%=user.userName%></td>
            <td title="<%=user.loginName%>"><%=user.loginName%></td>
            <td title="<%=user.employeeCode%>"><%=user.employeeCode%></td>
            <td title="<%=user.organize.organizeName%>"><%=user.organize.organizeName%></td>
            <td title="<%=user.positionName%>"><%=user.positionName%></td>
            <%if(user.status === 'ENABLE'){%>
            <td>激活</td>
            <%}else if(user.status === 'FORBIDDEN'){%>
            <td>冻结</td>
            <%}%>
        </tr>
        <%})%>
...
//                        }
            ...
            renderData: function () {
                ...
                $('.pe-stand-table-wrap').peGrid({
                    url: pageContext.rootPath + '/ems/examResult/manage/searchProcessResult',
