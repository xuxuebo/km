...
<div class="tbc-u-edit-face-panel" style="padding: 0;">
    <div id="previewPanel">
        ...
        <div class="preview-container">
            <img onerror="javascript:this.src='${resourcePath!}/web-static/proExam/images/default/default_face.png'" src="${(targetUrl)!'${resourcePath!}/web-static/proExam/images/default/default_face.png'}" class="jcrop-preview"
                 alt="Preview"/>
        </div>
    ...
    <div class="jc-demo-box" style="height:200px;width:360px;">
        <img src="${(sourceUrl)!'${resourcePath!}/web-static/proExam/images/default/default_face.png'}" id="target"
             style="display: none"/>
...
<form target="frm" class="tbc-u-re-upload-wrap upLoadForm" style="margin-top: 15px;"
    ...
    <input type="hidden" name="templateType" value="USER"/>
    <span>本地上传</span>
    <input type='file' accept="image/jpeg,image/png,image/jpg,image/gif"   hidefocus="hidefocus" name="uploadFile"
           onchange="uploadImg(this)"/>
</form>
...
<script type="text/javascript">
    function uploadImg(e,file) {
        ...
        if (sub !== 'jpg' && sub !== 'jpeg' && sub !== 'png' && sub !== 'bmp') {
            ...
            PEMO.DIALOG.tips({
                content: '图片格式仅支持jpg、jpeg、png、bmp',
                time:2000
            });
    ...
    function save() {
        ...
        PEBASE.ajaxRequest({
            ...
            success: function (data) {
                ...
                if (data.success) {
                    PEBASE.ajaxRequest({
                        url:'${ctx!}/uc/user/client/reviseFace',
                        data:{faceFileId:data.data.id,faceFileName:data.data.fileName,id:'${(userId)!}'},
                        success:function(data){
                            if(data.success){
                                PEMO.DIALOG.tips({
                                    content:'修改成功',
                                    end:function(){
                                        $('.pe-user-head-edit-btn', parent.document).children('img').attr('src', filePath);
                                        var index = parent.layer.getFrameIndex(window.name);
                                        parent.layer.close(index);
                                    }
                                });
