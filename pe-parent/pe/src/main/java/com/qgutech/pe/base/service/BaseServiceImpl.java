...
import static com.qgutech.pe.base.model.BaseModel.*;
...
public class BaseServiceImpl<T extends BaseModel> implements BaseService<T>, PeConstant {
    ...
    protected Conjunction getConjunction() {
        ...
        conjunction.add(Restrictions.eq(_corpCode, ExecutionContext.getCorpCode()));
        return conjunction;
    }
    ...
    private int update(String[] fields, T model) {
        ...
        builder.append(WHERE).append(_id).append(EQUAL).append(COLON + _id);
        builder.addParameter(processHqlMapKey(_id), model.getId());
    ...
    public Page<T> search(PageParam pageParam, Criterion condition, Order... orders) {
        ...
        if (StringUtils.isNotEmpty(sortName)) {
            ...
            criteria.addOrder(Order.desc(_createTime));
        }
