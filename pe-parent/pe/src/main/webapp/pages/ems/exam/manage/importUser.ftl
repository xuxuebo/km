...
<div class="pe-question-import-wrap">
    ...
    <div class="pe-import-tip-text">
        <p class="floatL">请先下载<strong style="color:#199ae2;">考生</strong>导入模板，根据导入模板的格式，添加您的内容，然后上传</p>
        <a class="pe-import-template-btn pe-btn pe-btn-green"
           href="${ctx!}/ems/exam/manage/exportTemplate?optType=${(exam.optType)!}&id=${(exam.id)!}"><span
...
<script>
    $(function () {
        var thisUpload = PEMO.UPLOAD({
            ...
            fileVal:'examUserFile',
            disableGlobalDnd: true,
            chunked: true,
            server: pageContext.rootPath + '/ems/exam/manage/importExamUser?examId=${(exam.id)!}&examArrangeId=${(examArrange.id)!}',
            fileNumLimit: 3000,
            fileSizeLimit: 10*1024 * 1024,
            fileSingleSizeLimit: 10*1024 * 1024,
            afterSuccessUploadContent: '<span class="pe-uploader-success-text">恭喜您学员信息导入成功！</span>',
            afterFailUploadContent: '<span class="pe-uploader-fail-text">学员信息导入失败，请修改后重新上传！</span><a href="${ctx}/ems/exam/manage/downloadErrorTemplate?examId=${(exam.id)!}&id=${(examArrange.id)!}" class="pe-down-load-doc">点击下载错误文档</a>'
        });
        ...
        thisUpload.onUploadAccept = function(t,r){
            if(r.success === 'true' || r.success){
                ...
                window.opener.refreshCount('${(examArrange.id)!}', 'USER');
                return true;
