...
<script type="text/template" id="detailDetailTableTemp">
    <table class="pe-stand-table pe-stand-table-default">
        ...
        <tr>
            ...
            <td>未通过</td>
            <%}%>
            <td><%=examResult.score || examResult.score === 0 ?Number(examResult.score).toFixed(1):'--'%></td>
            <%if (examResult.examCount && examResult.examCount <= 1){%>
            <td>--</td>
            <%}else if(!examResult.exam || examResult.exam.examType != 'OFFLINE'){%>
            <td><%=examResult.firstScore?Number(examResult.firstScore).toFixed(1):'--'%></td>
            <%}%>
            <td><%=examResult.examCount && examResult.examCount > 1?(examResult.examCount-1):'--'%></td>
            <%if(examResult.examCount && examResult.examCount>1){%>
...
//                        }
            ...
            renderData: function () {
                ...
                $('.pe-stand-table-wrap').peGrid({
                    url: pageContext.rootPath + '/ems/examResult/manage/searchReleaseResult',
                    formParam: $('#allDetailForm').serializeArray(),//表格上方表单的提交参数
                    tempId: 'detailDetailTableTemp',//表格模板的id
                    title: examType === 'OFFLINE' ? peOffLineTableTitle : peTableTitle, //表格头部的数量及名称数组;
                    onLoad: function () {
                    }
                });
            }
