        ...
        <div class="pe-manage-panel-head">
            <div class="pe-stand-filter-form">
                <div class="pe-stand-form-cell">
                    ...
                    <dl class="floatL" style="margin-right:20px;">
                    ...
                    <button type="button" class="pe-btn pe-btn-blue">筛选</button>
                </div>
            </div>
        </div>
    </form>
...
<script type="text/template" id="detailDetailTableTemp">
    <table class="pe-stand-table pe-stand-table-default">
        ...
        <%_.each(peData.rows,function(examMonitor,index){%>
        <tr>
            <%if (peData.tableTitles[0].name === 'ticket') {%>
            <td title="<%=examMonitor.ticket%>"><%=examMonitor.ticket%></td>
            <%}%>
            <td title="<%=examMonitor.user.userName%>"><%=examMonitor.user.userName%></td>
            <td title="<%=examMonitor.user.loginName%>"><%=examMonitor.user.loginName%></td>
            <td title="<%=examMonitor.user.employeeCode%>"><%=examMonitor.user.employeeCode%></td>
            <td title="<%=examMonitor.user.organize.organizeName%>"><%=examMonitor.user.organize.organizeName%></td>
            <td title="<%=examMonitor.user.positionName%>"><%=examMonitor.user.positionName%></td>
            <%if(examMonitor.user.status === 'ENABLE'){%>
            <td>激活</td>
            <%}else if(examMonitor.user.status === 'FORBIDDEN'){%>
            <td>冻结</td>
            <%}%>
            <%if(examMonitor.examStatus === 'WAIT_EXAM'){%>
            <td>待参加</td>
            <%}else if(examMonitor.examStatus === 'PROCESS'){%>
            <td>考试中</td>
            <%}else if(examMonitor.examStatus === 'MISS_EXAM'){%>
            <td>缺考</td>
            <%}else if(examMonitor.examStatus === 'PASS'){%>
            <td>通过</td>
            <%}else if(examMonitor.examStatus === 'NO_PASS'){%>
            <td>未通过</td>
            <%}else if(examMonitor.examStatus === 'MARKING'){%>
            <td>待评卷</td>
            <%}else if(examMonitor.examStatus === 'RELEASE'){%>
            <td>已发布</td>
            <%}else if(examMonitor.examStatus === 'WAIT_RELEASE'){%>
            <td>待发布</td>
            <%}%>
        </tr>
        <%})%>
...
//                        }
            ...
            renderData: function () {
                ...
                $('.pe-stand-table-wrap').peGrid({
                    url: pageContext.rootPath + '/ems/examResult/manage/searchMonitor',
