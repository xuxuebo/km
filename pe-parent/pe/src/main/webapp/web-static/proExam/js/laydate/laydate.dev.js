    ...
    if(!Dates.box){
        ...
        +'<div class="laydate_bottom">'
          +'<ul id="laydate_hms">'
            ...
            +'<li><input readonly class="laydate-time">:</li>'
            +'<li><input readonly  class="laydate-time">:</li>'
            +'<li><input readonly  class="laydate-time"></li>'
          +'</ul>'
...
Dates.events = function(){
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            ...
            if(!Dates.hasClass(this, as[1])){
                $(this).parents('table').find('td').removeClass('laydate_click');
    ...
    Dates.on(as.ok, 'click', function(){
        if(Dates.valid){
            var daySelected = $('.laydate_table').find('td.laydate_click');
            Dates.creation([daySelected.attr('y'), daySelected.attr('m'),daySelected.attr('d')]);
        }
    });
